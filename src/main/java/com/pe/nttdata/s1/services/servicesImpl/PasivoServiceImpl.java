package com.pe.nttdata.s1.services.servicesImpl;


import com.pe.nttdata.s1.entity.Pasivo;
import com.pe.nttdata.s1.services.PasivoService;
import com.pe.nttdata.s1.commons.GenericServiceImpl;
import com.pe.nttdata.s1.dao.PasivoRepository;


import com.fasterxml.jackson.core.JsonProcessingException;
import com.google.gson.Gson;
import com.mongodb.client.model.Filters;
import lombok.extern.slf4j.Slf4j;
import org.bson.types.ObjectId;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.mongodb.core.MongoTemplate;
import org.springframework.data.repository.CrudRepository;
import org.springframework.stereotype.Service;
import reactor.core.publisher.Mono;

import java.util.Objects;

@Service
@Slf4j
public class PasivoServiceImpl extends GenericServiceImpl<Pasivo, String> implements PasivoService {

    @Autowired
    private MongoTemplate mongoTemplate;

    @Autowired
    private PasivoRepository pasivoRepository;

    @Override
    public CrudRepository<Pasivo, String> getDao() {
        return pasivoRepository;
    }

    @Override
    public Mono<Pasivo> findById(String _id) throws JsonProcessingException, IllegalArgumentException {
        return Mono.just(new Gson().fromJson(Objects.requireNonNull(mongoTemplate
                .getCollection(mongoTemplate
                        .getCollectionName(Pasivo.class))
                .find(Filters
                        .eq("_id", new ObjectId(_id)))
                .first()).toJson(), Pasivo.class));
    }


    @Override
    public Void deleteById(String _id) {
        mongoTemplate
                .getCollection(mongoTemplate
                        .getCollectionName(Pasivo.class))
                .deleteOne(Filters
                        .eq("_id", new ObjectId(_id)));
        return null;
    }

}